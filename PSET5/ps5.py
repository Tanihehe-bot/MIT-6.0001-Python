...
class BeforeTrigger(TimeTrigger):
    def evaluate(self, story):
        ...
        trigger_date = self.time.replace(tzinfo=None)
        return story_date < trigger_date
...
def read_trigger_config(filename):
    ...
    for line in lines:
        ...
        if parts[0] == "ADD":
            for name in parts[1:]:
                trigger_list.append(trigger_map[name])
            ...
            if trigger_type == "TITLE":
                ...
                trigger_map[trigger_name] = NotTrigger(
                    trigger_map[parts[2]]
                )
            
            elif trigger_type == "AND":
                trigger_map[trigger_name] = AndTrigger(
                    trigger_map[parts[2]],
                    trigger_map[parts[3]]
                )
            
            elif trigger_type == "OR":
                trigger_map[trigger_name] = OrTrigger(
                    trigger_map[parts[2]],
                    trigger_map[parts[3]]
                )
